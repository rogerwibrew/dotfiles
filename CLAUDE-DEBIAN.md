...
with code in this repository's **Debian + dwm branch**.

**IMPORTANT**: This is the `debian-dwm` branch. For other setups see:
...
Debian Stable with dwm. The core concept: clone this repo on a fresh
Debian install, run the bootstrap script, and have a fully functional
tiling window manager with all configurations, scripts, and tools.

### Philosophy: Stability Over Features

This setup prioritizes:
- **Rock-solid stability**: Debian Stable, no rolling releases
...
- **OS**: Debian Stable (Bookworm 12.x or newer stable)
...
- dwm, dmenu, st (suckless tools)
- Small CLI/TUI tools when Debian version is too old
...
The dotfiles use **direct symlinking** via bootstrap-debian.sh:
...
Two machines with different displays:

- **kelvin** (desktop): Standard scaling, US keyboard
- **watt** (laptop): HiDPI display, GB keyboard
...
Understanding `bootstrap-debian.sh` execution:

1. **System validation**: Check Debian, user 'roger', set SCRIPT_DIR
2. **System update**: `apt update && apt upgrade`
3. **Install build dependencies**: Base build tools, Xorg libs
4. **Install apt packages**: Parse `packages-debian.txt`
...
- Fresh Debian install + clone + bootstrap = working dwm in under 1 hour
