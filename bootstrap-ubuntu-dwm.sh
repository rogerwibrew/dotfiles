...
# 7. Compile suckless tools (dwm, dmenu, slstatus)
...
if [[ "$DISTRIB_ID" != "Ubuntu" ]]; then
  error "This script is for Ubuntu, not $DISTRIB_ID. Use bootstrap-debian.sh for Debian."
fi

UBUNTU_VERSION="$DISTRIB_RELEASE"
info "Detected Ubuntu $UBUNTU_VERSION ($DISTRIB_CODENAME)"

# Check Ubuntu version (recommend 22.04+)
UBUNTU_MAJOR=$(echo "$UBUNTU_VERSION" | cut -d. -f1)
UBUNTU_MINOR=$(echo "$UBUNTU_VERSION" | cut -d. -f2)
if [ "$UBUNTU_MAJOR" -lt 22 ]; then
  warning "Ubuntu $UBUNTU_VERSION detected. This script is designed for Ubuntu 22.04+. Some packages may be outdated."
fi
...
if [ "$HOSTNAME" != "kelvin" ] && [ "$HOSTNAME" != "watt" ] && [ "$HOSTNAME" != "newton" ]; then
  warning "Unknown hostname: $HOSTNAME (expected 'kelvin', 'watt', or 'newton')"
...
info "Installing packages from packages-ubuntu-dwm.txt..."
...
PACKAGES=$(grep -v '^#' "$SCRIPT_DIR/packages-ubuntu-dwm.txt" | grep -v '^$' | \
  sed 's/#.*//' | tr '\n' ' ')
...
if [ -n "$PACKAGES" ]; then
  ...
  warning "No packages found in packages-ubuntu-dwm.txt"
fi
...
[ -f "$SCRIPT_DIR/.Xresources-newton" ] && \
  create_symlink "$SCRIPT_DIR/.Xresources-newton" "$HOME/.Xresources-newton"
